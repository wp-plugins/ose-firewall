			...
			<div class="modal-body">
				<form id='secManager-form' class="form-horizontal group-border stripped" role="form">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input type="text" name="secManager-email" value="" class="form-control" pattern="^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-zA-Z]{2,6}(?:\.[a-zA-Z]{2})?)$" required>
						</div>
					</div>
					<div class="form-group">
					...
					<input type="hidden" name="option" value="com_ose_firewall"> <input type="hidden" name="controller" value="adminemails"> <input type="hidden" name="action" value="saveSecManager"> <input
						type="hidden" name="task" value="saveSecManager"
					>
			
			</div>
			<div class="modal-footer">
				<label id="sec-warning-label" class="col-sm-6 control-label" style="display: none"><i id="sec-warning-message" class="fa fa-exclamation-triangle" style="display: none"></i></label>
				<div id="buttonDiv">
					<div class="form-group">
						<button type="submit" class="btn btn-sm">
							<i class="text-primary glyphicon glyphicon-save"></i> <?php oLang::_('SAVE'); ?></button>
					</div>
				</div>
			</div>
			</form>
