...
class centroraDropbox {
	...
	public function upload($file, $path = '',$overwrite=true){
		...
		if (!is_readable($file) or !is_file($file))
			throw new Centrora_DropboxException("Error: File \"$file\" is not readable or doesn't exist.");
	...
	public function search($path = '', $query , $fileLimit = 1000){
		if (strlen($query)>=3)
			throw new Centrora_DropboxException("Error: Query \"$query\" must three characters long.");
	...
	public function oAuthAuthorize() {
		...
		if ($status>=200 and $status<300 and 0==curl_errno($ch) ) {
			...
			throw new Centrora_DropboxException($message);
		}
		curl_close($ch);
		return array( 'authurl'		   => self::API_WWW_URL . self::API_VERSION_URL . 'oauth/authorize?oauth_token='.$oauth_token['oauth_token'],
...
class Centrora_DropboxException extends Exception {
}
?>